...
#include "JonaModule.h"

extern Mesh cube;
extern Mesh Sphere;
void JonaModule::start()
{
...
void JonaModule::update(float elapsed)
{

	glm::vec3 euler = quad1.getEulerAngles();
	quad1.setPosition(glm::vec3(0, 0, 0));
	quad1.setScale(1.0f);
	quad1.setEulerAngles(glm::radians(euler.x + glfwGetTime() * 10), euler.y, euler.z);
...
void JonaModule::cameraController()
{
	front = glm::vec3(0, 0, 1);
...
void JonaModule::renderDebug()
{

	Engine::get().setModelObjectConstants(quad1.asMatrix(), glm::vec4(1.0f, 0.0f, 1.0f, 1.0f));
	quad.activateAndRender();
}


JonaModule::JonaModule()
{
}
